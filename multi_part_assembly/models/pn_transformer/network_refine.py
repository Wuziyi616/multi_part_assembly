...
class PNTransformerRefine(PNTransformer):
    ...
    def _init_corr_pos_enc(self):
        ...
        corr_pos_encs = _get_clones(corr_pos_enc, self.refine_steps)
        return corr_pos_encs

    def _init_corr_module(self):
        pos_enc_dims = self.cfg.model.transformer_pos_enc
        dim = self.pc_feat_dim + pos_enc_dims[-1]
    ...
    def forward(self, data_dict):
        ...
        for i in range(self.refine_steps):
            ...
            pos_enc = self.corr_pos_enc[i](pose)
            in_feats = torch.cat([part_feats, pos_enc], dim=-1)
            # transformer feature fusion
            valid_mask = (part_valids == 1)
            corr_feats = self.corr_module[i](in_feats, valid_mask)  # [B, P, C]
            # MLP predict poses
            if self.pose_pc_feat:
